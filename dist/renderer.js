(() => {
  // node_modules/@notmike101/zcode-extension-sdk/dist/esm/renderer.js
  function defineRendererExtension(extension) {
    return extension;
  }
  ...
  function mountDiscoveryDashboard(container, context) {
  ...
  `;
    ...
    let cachedProviders = [];
    let currentDiscoveredModels = [];
    const providerSelect = root.querySelector("#target-provider-select");
    const baseUrlInput = root.querySelector("#manual-base-url");
    const apiKeyInput = root.querySelector("#manual-api-key");
    const fetchBtn = root.querySelector("#manual-fetch-btn");
    const statusSpan = root.querySelector("#manual-fetch-status");
    const resultArea = root.querySelector("#manual-models-result");
    const countHeader = root.querySelector("#models-count-header");
    const tagsContainer = root.querySelector("#models-tags-container");
    const saveBtn = root.querySelector("#save-models-btn");
    const loadProviders = async () => {
      try {
        const res = await context.ipc.invoke("list-providers");
        if (!res.success) {
        ...
        providerSelect.innerHTML = `<option value="">-- Select target provider --</option>` + cachedProviders.map((p) => `<option value="${p.id}">${p.name} (${p.models.length} models)</option>`).join("");
        if (prevSelected) {
    ...
    fetchBtn.onclick = async () => {
      ...
      try {
        const res = await context.ipc.invoke("fetch-models", { baseUrl, apiKey });
        if (!res.success) {
    ...
    saveBtn.onclick = async () => {
      ...
      try {
        const res = await context.ipc.invoke("save-provider-models", {
...
//# debugId=0B3BA6688B51574264756E2164756E21
